...
public class OldZ3Solver {
    ...
    public AdditiveUtilitySpace estimatePreferences(List<Bid> bidRanking, Domain domain) {
        try (SolverContext context = SolverContextFactory.createSolverContext(
            ...
            initialiseArrays(domain.getIssues().size(), bidRanking.size());
            ...
            for(int i = 0; i < bidRanking.size(); i++) {
                bidUtilities[i] = nmgr.makeVariable("bid-" + i);
                Bid bid = bidRanking.get(i);
